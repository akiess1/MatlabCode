...
% number:
numGrps = 0;           % number of initial groups. enter 0 for no groups (random distribution)
Nb = 100; %50*numGrps;        % initial number of bacteria
% align:
aligngrps = 0;          % 1 for aliging groups in row (for sticky sims) else random
grpsep = 2;             % for two aligned groups (for sticky sims)
% types: (1 = true)
sticky = 0;        % sticky bacteria do not advect with flow
...
graphics = 1;            % 0 = don't display graphics (for crc runs); 1 = display on
saveVid = 0;             % enter 1 to save video after simulation ends
filename = sprintf('const_shear%d',vmax);        % name of file to save video
...
pcheck = printprogsum(xmin,xmax,Nx,ymin,ymax,Ny,...
...
if pcheck == 1
    ...
    fprintf('*** RUN FAILED *** \n');
    fprintf('*** Invalid parameter choices *** \n');
end
